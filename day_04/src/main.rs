...
#[test]
fn test_duplicates_in_line() {
	assert_eq!(has_duplicates("aa bb cc dd ee"), false);
	assert_eq!(has_duplicates("aa bb cc dd aa"), true);
	assert_eq!(has_duplicates("aa bb cc dd aaa"), false);
	assert_eq!(has_duplicates("aa aa aa aa aa"), true);
}
...
fn test_count_valid() {
	...
	assert_eq!(count_valid(input), 2);

	let input2 = "oaoe rxeq vssdqtu xrk cjv yaoqp loo
	...
	assert_eq!(count_valid(input2), 386);
}

fn has_duplicates(pass: &str) -> bool {
...
fn count_valid(data: &str) -> u32 {
	data.lines()
		.filter(|line| line.trim().len() > 1 )
		.filter(|pass| !has_duplicates(pass) )
		.count() as u32
}

fn count_valid2(data: &str) -> u32 {
	data.lines()
		.filter(|line| line.trim().len() > 1 )
		.filter(|pass| !has_duplicates(pass) )
		.filter(|pass| !contains_anagrams(pass) )
		.count() as u32
}


#[test]
fn test_contains_anagram() {
...
fn contains_anagrams(pass: &str) -> bool {
	use std::collections::HashMap;

	let sorted = pass.split_whitespace()
		.map(|word| {
			let mut w = word.to_owned().into_bytes();
			w.sort();
...
fn main() {
	...
	println!("part 2: {}", count_valid2(input));
}
